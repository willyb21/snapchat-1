...
abstract class SnapchatAgent {
	...
	public function get($endpoint) {
		...
		$iplist=file("/srv/www/snapchat.am/public_html/api/iprivate");
		$iprand=rand(1,$numips);
				$filePath = '/srv/www/snapchat.am/public_html/libs/number';
		$f = fopen($filePath, 'r+');
		$ipNumber = fgets($f);
		fclose($f);
		$sleep=rand(2,3);
		$ip=$iplist[$ipNumber];
		$spoof="Mozilla/5.0 (Windows; U; MSIE 9.0; WIndows NT 9.0; en-US))";
		sleep($sleep);


		$ch = curl_init();
		$CURL_OPTIONS["CURLOPT_USERAGENT"]=$spoof;
